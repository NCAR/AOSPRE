...
!---------------------------------------------------------------------------------------------
  ...
  subroutine find_all_wrf_times(wrf_glob_pattern, conv_minute, wrf_reference_time, wrf_files, wrf_times, wrf_xtimes, wrf_file_index, wrf_time_index, dx, idim, jdim)
    ...
    open(10,file=".thing", status='old', form='formatted', action='read', iostat=io_status)
    if (io_status /= 0) then
...
!---------------------------------------------------------------------------------------------
  ...
  subroutine projection_from_wrf_output(wrf_file, proj)
    ...
    !real(kind=RKIND), allocatable, dimension(:,:), intent(out) :: mapfac

    integer :: ncid, lat_dimid, nlat, nlon
    integer :: ierr
    integer :: varid
    ...
    !call error_handler ( ierr, "Problem inq_dimid for 'west_east'" )
    !ierr = nf90_inquire_dimension ( ncid, west_east_dimid, len=self%ni )
    !call error_handler ( ierr, "Problem inquiring dimension west_east." )
    ...
    !ierr = nf90_get_var(ncid, varid, mapfac, start=(/1,1/))    ! Reading the map factor (BWK, 10/24/2025)
